...
async def instagram(email, client, out):
    ...
    try:
        freq = await client.get("https://www.instagram.com/accounts/emailsignup/", headers=headers, follow_redirects=True)
        token = freq.cookies.get("csrftoken") or client.cookies.get("csrftoken")
        if not token:
